%% Cálculo da direção do somatório de ondas com diferentes frequencias e direções
clear,clc,close all

onda30=load('registro30f.txt'); %Tp=12 segundos
onda60=load('registro150f1.txt'); %Tp=4 segundos
...
%  "    8-velzzz - velocidade vertical (Dpot.vel/Dz) (utilizar esta pois varia com o z e é mais parecida com a velz quando z=0)
%  "    9-pr - pressão (peso especifico = 1.025 g/cm^3)

%% Muda a direção com flipud (nao muda a direcao calculada. pq?)
% onda30=flipud(onda30);
% onda60=flipud(onda60);
% somaonda=flipud(somaonda);
...
set(get(AX(1),'Ylabel'),'String','Eta (m)') ;
set(get(AX(2),'Ylabel'),'String','Pressão (mbar)'); 
title('Série da Elevação e Pressão para ondas de 60 graus e Tp de 4 segundos') 
...
title('Auto-Espectro'),xlabel('Frequência (Hz)'),ylabel('m²/Hz')
legend('Pressão')
subplot(2,1,2)
hold on
...
subplot(2,1,1)
plot(deltaf,aaeta(:,2))
title('Auto-Espectro da Elevação'), xlabel('Frequência (Hz)'),ylabel('m²/Hz')
subplot(2,1,2)
plot(deltaf,diraz)
